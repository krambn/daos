...
again:
	for (i = 0, rc = 0;
		...
		rsrvd_scm = dru->dru_scm_ptr;
		if (rsrvd_scm != NULL) {
			rc = vos_publish_scm(cont, &rsrvd_scm, err == 0, NULL);
			 ...
			 */
			if (rc == 0 || err != 0)
				rc = vos_publish_blocks(cont,
						&dru->dru_nvme_list, err == 0,
						VOS_IOS_GENERIC, NULL);

			if (err != 0)
				D_FREE(dru->dru_scm_ptr);
		}
	}
...
static void
vos_tx_commit_cb(void *data, bool noop)
{
	struct dtx_handle	*dth = data;
	int			 i;
...
vos_tx_end(struct dtx_handle *dth, struct umem_instance *umm, int err)
{
	...
	err = umem_tx_add_callback(umm, vos_txd_get(), TX_STAGE_ONCOMMIT,
				   vos_tx_commit_cb, dth);
	if (err != 0)
		D_ERROR("Failed to add TX callback for "DF_DTI": "
			DF_RC"\n", DP_DTI(&dth->dth_xid), DP_RC(err));
