...
#define SCHED_SPACE_AGE_MAX	5000	/* 5000 msecs */
		     ...
		     bool purge)
{
	...
	if (scm_left < (SCM_TOTAL(&vps) * 1 / 10) ||
		...
		spi->spi_space_pressure = 2;	/* severe space pressure */
	else if (scm_left < (SCM_TOTAL(&vps) * 3 / 10) ||
		 nvme_left < (NVME_TOTAL(&vps) * 3 / 10))
		spi->spi_space_pressure = 1;	/* light space pressure */
	else
		spi->spi_space_pressure = 0;	/* no space pressure */
...
process_pool_cb(d_list_t *rlink, void *arg)
{
	...
	switch (rc) {
	case 2:	/* under severe space pressure */
		if ((gc_max / 2) > 0)
			io_max = min(io_max, gc_max / 2);
		else if (gc_max)
			io_max = min(io_max, gc_max);
		break;
	case 1: /* under light space pressure */
		if (gc_max)
			io_max = min(gc_max, io_max);
		break;
	case 0:	/* no space pressure, throttle gc requests */
