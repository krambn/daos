...
func (sqr *SmdQueryResp) addHostResponse(hr *HostResponse) (err error) {
	...
	for _, rResp := range pbResp.GetRanks() {
		...
		for _, rPool := range rPools {
			matched := false
			for _, hPool := range hs.SmdInfo.Pools {
				if hPool.UUID == rPool.UUID {
					hPool.Ranks = append(hPool.Ranks, rank)
					hPool.Blobs = append(hPool.Blobs, rPool.Blobs...)
					matched = true
					break
				}
			}
			...
			rPool.Ranks = []system.Rank{rank}
			hs.SmdInfo.Pools = append(hs.SmdInfo.Pools, rPool)
		}
...
func SmdQuery(ctx context.Context, rpcClient UnaryInvoker, req *SmdQueryReq) (*SmdQueryResp, error) {
	...
	if req.SetFaulty && len(req.HostList) > 1 {
